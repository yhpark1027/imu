        ...
        while not rospy.is_shutdown():
            try:
                ...
                if norm > 1e-9:
                    ...
                    cy = math.cos(tilt_compensated_heading - last_yaw)
                    sy = math.sin(tilt_compensated_heading - last_yaw)
                    cp = math.cos(pitch * self.RAD_TO_DEG)
                    sp = math.sin(pitch * self.RAD_TO_DEG)
                    cr = math.cos(roll * self.RAD_TO_DEG)
                    sr = math.sin(roll * self.RAD_TO_DEG)
                    ...
                    msg.orientation_covariance = [0, 0, 0,  # covariance on x axis
                                                  0, 0, 0,  # covariance on y axis
                                                  0, 0, 0]  # covariance on z axis

                    msg.angular_velocity = Vector3(rate_gyr_x * self.GYRO_TO_RADS, rate_gyr_y * self.GYRO_TO_RADS,
                                                   rate_gyr_z * self.GYRO_TO_RADS)
                    msg.angular_velocity_covariance = [0, 0, 0,  # covariance on x axis
                                                       0, 0, 0,  # covariance on y axis
                                                       0, 0, 0]  # covariance on z axis
                    ...
                    msg.linear_acceleration_covariance = [0, 0, 0,  # covariance on x axis
                                                          0, 0, 0,  # covariance on y axis
                                                          0, 0, 0]  # covariance on z axis
